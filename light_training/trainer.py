...
class Trainer:
    ...
    def validation_single_gpu(self,val_name, val_dataset):
        # print(val_dataset[3])
        if self.ddp:
        ...
        for idx, batch in tqdm(enumerate(val_loader), total=len(val_loader)):
            ...
            with torch.no_grad():
                val_out = self.validation_step(val_name[idx],batch)
        ...
        if self.local_rank == 0:
            with tqdm(total=len(loader)) as t:
                ...
                for idx, batch in enumerate(loader):
                    # print("batch.type",type(batch[0][0]))
                    # batch [0][0]image [0][1]label_text [1][0]mask
                    # print("batch.shape",batch[1][0].shape)
