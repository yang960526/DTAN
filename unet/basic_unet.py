...
class BasicUNetEncoder(nn.Module):
    ...
    def __init__(
        ...
        self.conv_0 = TwoConv(2, 3, features[0], act, norm, bias, dropout)
        self.down_1 = Down(spatial_dims, fea[0], fea[1], act, norm, bias, dropout)
        self.down_2 = Down(spatial_dims, fea[1], fea[2], act, norm, bias, dropout)
        self.down_3 = Down(spatial_dims, fea[2], fea[3], act, norm, bias, dropout)
        self.down_4 = Down(spatial_dims, fea[3], fea[4], act, norm, bias, dropout)
    ...
    def forward(self, x: torch.Tensor):
        ...
        x3 = self.down_3(x2)
        x4 = self.down_4(x3)
        # print("x.shape",x.shape) # 4 4 96 96 96
        # print("x0,x1,x2,x3,x4",x0.shape,x1.shape,x2.shape,x3.shape,x4.shape)
        # [32, 64, 256, 256] [32, 128, 128, 128] [32, 256, 64, 64] [4, 512, 32, 32] [4, 1024, 16, 16]
